...
DATA_DIR = '../datasets'
OUTPUT_DIR = 'output'
DATASETS = ['easy', 'hard', 'bio', 'finance', 'speech', 'vision', 'iris']
...
for dataset in DATASETS:
    ...
    unformatted_cmd = 'python3 classify.py --data %s --mode train --model-file %s --algorithm %s --cluster-lambda %f --clustering-training-iterations %d'
    cmd = unformatted_cmd % (data, model_file, ALGORITHM, 0.0, 5)
    os.system(cmd)
    ...
    for subset in ['train', 'dev', 'test']:
        ...
        if subset != 'test':
            ...
            #cmd = 'python3 ../clustering_python_scripts/number_clusters.py %s' % (predictions_file)
            #os.system(cmd)
