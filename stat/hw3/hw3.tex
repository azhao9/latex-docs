...
\begin{itemize}
	...
	\item[46.] Use the central limit theorem to sketch the approximate sampling distribution of $\bar{Y}_R$ for $n=64$ for the population of hospitals. Compare to the approximate sampling distribution of $\bar{Y}.$
		\begin{soln}
			...
			We also have 
			\begin{align*}
				\var(\bar{Y}_R) &\approx \frac{1}{n} \left( 1-\frac{n-1}{N-1} \right)(r^2\sigma_x^2+\sigma_y^2-2r\rho\sigma_x\sigma_y) \\
				& \approx 22.4
			\end{align*}

			so $\bar{Y}_R\sim N(813.4, 22.4).$ We also know the sampling distribution for $\bar{Y}$ is roughly normal with $\mu=\mu_y$ and $\sigma^2=\sigma_y^2/n,$ which is $\bar{Y}\sim N(814.6, 5435)$, and the two distributions are plotted below:
			\begin{center}
				\includegraphics[width=14cm]{y_bar_plot.png}
			\end{center}

			The sampling distribution of $\bar{Y}$ is approximately a normal distribution with mean $\mu_y$ and variance $\sigma_y^2/n,$ which for.
		\end{soln}<++>
